...
main() {
    ...
    (sudo apt install python3-venv -y && python3 -m venv SNI && source SNI/bin/activate && pip install panda && pip install tabulate && pip install re && pip install subprocess)
